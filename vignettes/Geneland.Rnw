%\VignetteIndexEntry{Geneland: clustering of georeferenced genetic data}
...
\begin{document}
%% (\SweaveOpts{concordance=TRUE} removed: the vignette is built with knitr,
%%  which does not define that macro. The document contains no code chunks.)
...
See \url{https://CRAN.R-project.org/ }.
See \url{ https://en.wikipedia.org/wiki/R\_(programming\_language)} for details.\\
...
See \url{ https://CRAN.R-project.org/sources.html } for details and update.
 

Useful sources of information include the various R manuals \footnote{available from \url{https://CRAN.R-project.org/}} and 
...
\paragraph{What is new in version 5.0.0.}
The statistical model and the MCMC sampler are unchanged; for a given seed,
\texttt{MCMC()} reproduces the output of version 4.9.2 exactly. What changed is
the interface. \texttt{Geneland.GUI()} now opens a web-based interface built
with \texttt{shiny} (the previous Tk interface is still available as
\texttt{Geneland.GUI.tcltk()}, and is the one documented in
Section~\ref{sec:gui}). Figures can now also be produced as
\texttt{ggplot2} objects: \texttt{read\_geneland()} reads a run directory, and
\texttt{autoplot()} on the result returns a plot that can be modified and
saved by the caller. The \texttt{Plot*} functions described in this document
are unchanged. The file \texttt{NEWS.md} in the installed package lists all
the changes, including a number of bug fixes.
...
\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section[Examples with the GUI]{Examples of data analysis using the graphical user interface (GUI)}\label{sec:gui}

\subsection{Launching the GUI}

\noindent{\ggr\textbf{Note for version 5.0.0 and later.}} {\sc Geneland}
now ships two graphical interfaces. \texttt{Geneland.GUI()} launches a
web-based interface built with \texttt{shiny}, which covers the standard
workflow (load data, choose a model, run the sampler, inspect the chain, view
and export the maps). This chapter documents the older Tk interface, which is
unchanged and remains available as \texttt{Geneland.GUI.tcltk()}; it still
provides features the new interface does not yet have, namely parallel
multiple runs, F-statistics, data simulation and the hybrid zone tools. The
screenshots below are all of the Tk interface.\\

\begin{itemize}
\item launch R
\item load {\sc Geneland} by typing \texttt{library(Geneland)} in the R prompt
\item launch the Tk GUI by typing \texttt{Geneland.GUI.tcltk()} in the R prompt.
\end{itemize}
...
On the various formats and how to load them in R: \url{https://CRAN.R-project.org/view=Spatial}\\

Loading data, converting coordinates, plotting: \url{https://sites.google.com/site/spatialr}\\

Data (climate, landcover, coastal lines, administrative boundaries: \url{https://data.apps.fao.org/catalog/}\\
...
named wfs\_coas.shp from \url{https://data.apps.fao.org/catalog/}).
